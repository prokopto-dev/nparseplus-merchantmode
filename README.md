...
The picker above the tabs scopes the **whole window**, and that is the plugin's
...
it. That one gets a ⚠ on the item's own name, in the theme's warning colour,
...
![The settings page: pause between macro lines, max socials, poll interval, staleness threshold, abbreviation and ID-column toggles, and line prefix](assets/screenshots/settings--merchant-mode.png)
...
budget, one of every ID badge, a month-stale mule, an evening of auctions that
disagree with each other, and a filter rule of each kind including one that
catches nothing — so the README shows the states that are hardest to describe in
prose. `tests/test_capture_screenshots.py` asserts it still does, so the
pictures can't quietly stop matching the words.
