...
![The Sell tab: items pooled from two characters, the Where column naming which alt holds each one, a stale dump flagged, and the byte budget at 250 of 255](assets/screenshots/window--sell.png)
...
### Want — items you don't own, with IDs and prices attributed

![The Want tab: four wanted items with ID badges and suggested buy prices, each labelled with the source it came from](assets/screenshots/window--want.png)
...
### Prices — what the channel has actually been paying

![The Prices tab: eight recent auction observations with prices, WTS/WTB side, and seller](assets/screenshots/window--prices.png)

Built from live `/auction` traffic as it scrolls past — both halves of a line,
so `WTS foo 5k WTB bar 2k` lands in the table as two rows on opposite sides.
...
left blank. Two sources feed it, and live data wins:
...
2. **PigParse's six-month average** — steadier, but slower to notice a market
   that's moved. Used when nothing has been seen live.
...
2. **Sell** tab → *Load inventory dump…* (once per character) → tick what you're
   selling.
